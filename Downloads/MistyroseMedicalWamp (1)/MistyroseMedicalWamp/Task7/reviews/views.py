...
class ReviewListCreateView(generics.ListCreateAPIView):
    ...
    ordering_fields = ['created_at', 'rating', 'likes_count', 'interaction_score']
    ordering = ['-created_at']
    ...
    def get_queryset(self):
        ...
        # تصفية حسب عدد النجوم المحددة
        rating_filter = self.request.query_params.get('stars')
        if rating_filter:
            try:
                stars = int(rating_filter)
                if 1 <= stars <= 5:
                    queryset = queryset.filter(rating=stars)
            except ValueError:
                pass

        # تصفية حسب المنتج المحدد
        product_filter = self.request.query_params.get('product_id')
        if product_filter:
            try:
